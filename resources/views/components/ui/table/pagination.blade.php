@if ($paginator->hasPages())
    <div class="flex justify-between items-center uppercase sm:grid-cols-9 print:hidden">
        <div class="flex justify-between w-full md:hidden">
            ...
            @else
                <button type="button" wire:click="previousPage" wire:loading.attr="disabled" wire:offline.attr="disabled" wire:loading.class="cursor-wait" class="flex justify-center items-center ps-1.5 pe-3 py-1 gap-1 rounded-md text-white hover:transition-colors hover:duration-500 bg-primary-500 dark:bg-secondary-500 hover:bg-primary-700 dark:hover:bg-secondary-700" title="Previous">
                    <span class="material-symbols-outlined">keyboard_double_arrow_left</span>
                    <span>Previous</span>
                </button>
            @endif
            @if ($paginator->hasMorePages())
                <button type="button" wire:click="nextPage" wire:loading.attr="disabled" wire:offline.attr="disabled" wire:loading.class="cursor-wait" class="flex justify-center items-center ps-3 pe-1.5 py-1 gap-1 rounded-md text-white hover:transition-colors hover:duration-500 bg-primary-500 dark:bg-secondary-500 hover:bg-primary-700 dark:hover:bg-secondary-700" title="Next">
                    <span>Next</span>
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li>
                                <button type="button" disabled class="flex justify-center items-center px-2.5 py-1 hover:transition-colors hover:duration-500 bg-primary-500 dark:bg-secondary-500 text-white dark:text-gray-200 rounded-md cursor-not-allowed">
                                    {{ $page }}
                                </button>
                            </li>
                        @else
