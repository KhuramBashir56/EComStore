            ...
            @forelse($brands as $brand)
                <x-ui.table.tr wire:key="brand-{{ $brand->ref_id }}">
                    ...
                    <x-ui.table.td class="text-center">
                        <x-ui.table.actions>
                            @if ($brand->status === 'published')
                                <x-ui.buttons.transparent-icon-button type="button" :button="__('red')" wire:click="unPublishBrand('{{ $brand->ref_id }}')" wire:confirm="Are you sure you want to unpublish this brand?" :title="__('Un Publish')" :icon="__('visibility_off')" class="rounded-md" />
                            @else
                                <x-ui.buttons.transparent-icon-button type="button" :button="__('green')" wire:click="publishBrand('{{ $brand->ref_id }}')" wire:confirm="Are you sure you want to publish this brand?" :title="__('Publish')" :icon="__('visibility')" class="rounded-md" />
                            @endif
                            <x-ui.links.transparent-icon-link :link="__('default')" wire:navigate href="{{ route('admin.products.brands.details', ['brand' => $brand->ref_id]) }}" :title="__('View Details')" :icon="__('info')" class="rounded-md" />
                            <x-ui.links.transparent-icon-link :link="__('blue')" wire:navigate href="{{ route('admin.products.brands.edit', ['brand' => $brand->ref_id]) }}" :title="__('View Details')" :icon="__('edit')" class="rounded-md" />
                            <x-ui.buttons.transparent-icon-button type="button" :button="__('red')" wire:click="deleteBrand('{{ $brand->ref_id }}')" wire:confirm="Are you sure you want to delete this brand?" :title="__('Delete')" :icon="__('delete')" class="rounded-md" />
                        </x-ui.table.actions>
                    </x-ui.table.td>
                </x-ui.table.tr>
            @empty
