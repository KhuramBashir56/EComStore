<x-slot:title>{{ __('Categories') }}</x-slot>
    ...
    <x-ui.table>
        ...
        <x-ui.table.body>
            @forelse($categories as $category)
                <x-ui.table.tr wire:key="category-{{ $category->ref_id }}">
                    <x-ui.table.td>
                    ...
                    <x-ui.table.td class="text-center">
                        <x-ui.table.actions>
                            @if ($category->status === 'published')
                                <x-ui.buttons.transparent-icon-button type="button" :button="__('red')" wire:click="unPublishCategory('{{ $category->ref_id }}')" wire:confirm="Are you sure you want to unpublish this category?" :title="__('Un Publish')" :icon="__('visibility_off')" class="rounded-md" />
                            @else
                                <x-ui.buttons.transparent-icon-button type="button" :button="__('green')" wire:click="publishCategory('{{ $category->ref_id }}')" wire:confirm="Are you sure you want to publish this category?" :title="__('Publish')" :icon="__('visibility')" class="rounded-md" />
                            @endif
                            <x-ui.links.transparent-icon-link :link="__('default')" wire:navigate href="{{ route('admin.products.categories.details', ['category' => $category->ref_id]) }}" :title="__('View Details')" :icon="__('info')" class="rounded-md" />
                            <x-ui.links.transparent-icon-link :link="__('blue')" wire:navigate href="{{ route('admin.products.categories.edit', ['category' => $category->ref_id]) }}" :title="__('Edit')" :icon="__('edit')" class="rounded-md" />
                            <x-ui.buttons.transparent-icon-button type="button" :button="__('red')" wire:click="deleteCategory('{{ $category->ref_id }}')" wire:confirm="Are you sure you want to delete this category?" :title="__('Delete')" :icon="__('delete')" class="rounded-md" />
                        </x-ui.table.actions>
                    </x-ui.table.td>
                </x-ui.table.tr>
            @empty
