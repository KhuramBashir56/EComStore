<x-slot:title>{{ __('Edit Category') }}</x-slot>
<section class="grid gap-4">
    <x-ui.card class="w-full rounded-xl p-4 sm:grid-cols-2">
        <div class="sm:col-span-2 mb-4">
            @if ($thumbnail)
                <x-thumbnail :alt="__('Category Thumbnail')" :path="$thumbnail->temporaryUrl()" class="w-full sm:max-w-56 aspect-square" />
            @else
                <x-thumbnail :alt="__('Category Thumbnail')" :path="$old_thumbnail ? asset(config('filesystems.storage') . $old_thumbnail) : asset('assets/images/card-image.svg')" class="w-full sm:max-w-56 aspect-square" />
            @endif
        </div>
        ...
        <div class="flex justify-end gap-4 mt-4 sm:col-span-2">
            ...
            <x-ui.buttons.button type="button" :button="__('green')" wire:click="updateCategory" :title="__('Update')" class="rounded-md" />
        </div>
    </x-ui.card>
</section>
